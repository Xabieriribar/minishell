...
int open_redir_append(char *filename, int append_count, int *updated_fd_append)
{
    if (append_count > 0)
        close(*updated_fd_append);
    *updated_fd_append = open(filename, O_WRONLY | O_CREAT | O_APPEND);
    if (*updated_fd_append < 0)
        perror("Open failed when output redirections");
    return (update_fd_out);
}
int open_redir_out(char *filename, int out_count, int *updated_fd_out)
{
    if (out_count > 0)
        close(*update_fd_out);
    *updated_fd_out = open(filename, O_WRONLY | O_CREAT | O_TRUNC);
    if (updated_fd_out < 0)
